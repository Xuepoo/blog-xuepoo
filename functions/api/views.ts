...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  if (request.method === 'GET') {
    try {
      if (slug) {
        // 获取单篇文章阅读数
        const result = await env.DB.prepare('SELECT views FROM post_views WHERE slug = ?')
          .bind(slug)
          .first<{ views: number }>();
        ...
        const { results } = await env.DB.prepare('SELECT slug, views FROM post_views').all<{
          slug: string;
          views: number;
        }>();
    ...
    try {
      ...
      if (logCheck) {
        ...
        const result = await env.DB.prepare('SELECT views FROM post_views WHERE slug = ?')
          .bind(slug)
          .first<{ views: number }>();
        return new Response(JSON.stringify({ views: result?.views || 0, skip: 'cooldown' }), {
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      const statements = [
        env.DB.prepare('INSERT OR REPLACE INTO view_logs (ip, slug) VALUES (?, ?)').bind(ip, slug),
        env.DB.prepare(
          'INSERT INTO post_views (slug, views) VALUES (?, 1) ON CONFLICT(slug) DO UPDATE SET views = views + 1, updated_at = CURRENT_TIMESTAMP',
        ).bind(slug),
        env.DB.prepare('SELECT views FROM post_views WHERE slug = ?').bind(slug),
      ];

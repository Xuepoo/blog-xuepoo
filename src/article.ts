 ...
 * heaviest thing the app can load (~380KB minified). The blog's list pages
 * render plain frontmatter descriptions, so markdown is only needed when an
 * article page is opened. The class is therefore built lazily behind a dynamic
 * import: `bun build --splitting` emits markdown as its own chunk that is
 * fetched on first article navigation, keeping the eager bundle to core + ui.
 */

/** The markdown entity as the article page consumes it. */
